...
function Workspace({user,recovery,onRecovered,authError}:{user:User|null;recovery:boolean;onRecovered:()=>void;authError:string}){
 ...
 useEffect(()=>{if(recovery)navigate('auth');else if(user&&workspace.ready&&(page==='auth'||page==='home'&&(workspace.importing||location.search.includes('code='))))navigate(workspace.importing?'wizard':'drawers');},[user,recovery,navigate,page,workspace.ready,workspace.importing]);
 useEffect(()=>{if(workspace.importedSaved&&!recovery)navigate('drawers');},[workspace.importedSaved,recovery,navigate]);
 ...
 const saveDrawer=async()=>{if(!user){try{await workspace.beforeAuth(true);setNotice('');navigate('auth');}catch{setNotice('Your drawer could not be cached. Keep this page open and try saving again.');}return;}if(await workspace.save())navigate('drawers');};
 ...
 {(page==='auth'||page==='drawers'&&!user)&&<Auth savingDrawer={!!draft.pendingAccountSave} recovery={recovery} onRecovered={onRecovered} onBeforeAuth={()=>workspace.beforeAuth()} onClose={()=>navigate('wizard')}/>}
 ...
 <div hidden={doc.step!==2}><Place onSave={saveDrawer} drawerName={doc.name} onRename={name=>change(old=>({...old,name}))} key={draft.id??'new'} drawer={drawer} groups={doc.groups} onChange={setGroups} onMeasure={()=>change(old=>({...old,step:1}))} layout={doc.layout} setLayout={setLayout} usableHeightMm={doc.usableHeightMm} setUsableHeightMm={setHeight}/></div>{(user||workspace.status)&&<div className="drawer-save-status"><span role="status" aria-live="polite">{workspace.status||(user?'Ready':'')}</span>{user&&draft.dirty&&<button className="place-button" onClick={workspace.retry}>Retry save</button>}{workspace.conflict&&<><button className="place-button" onClick={workspace.saveCopy}>Save as new drawer</button><button className="place-button" onClick={()=>{if(draft.id)void workspace.open({id:draft.id});}}>Reload saved drawer</button></>}</div>}</div>}
