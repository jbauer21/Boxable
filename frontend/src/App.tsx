...
function Workspace({user,recovery,onRecovered,authError}:{user:User|null;recovery:boolean;onRecovered:()=>void;authError:string}){
 ...
 <div hidden={doc.step!==2}><Place key={draft.id??'new'} drawer={drawer} groups={doc.groups} onChange={setGroups} onMeasure={()=>change(old=>({...old,step:1}))} layout={doc.layout} setLayout={setLayout} usableHeightMm={doc.usableHeightMm} setUsableHeightMm={setHeight}/></div></div>}
