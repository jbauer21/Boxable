...
export function specsForGroups(groups: ItemGroup[], depth: number, cols: number, rows: number) {
  const maxHeight = Math.min(20, Math.floor((depth - 5) / 7));
  ...
  for (const g of groups) {
    ...
    const made = specsForGroup(g,maxHeight,maxFootprint);
    if (!made.length) skipped.push(g.name);
    made.forEach((s,i) => { for (let j=0;j<s.quantity;j++) specs.push({...s,name:g.name,id:`${g.id}:${i}:${j}`,quantity:1}); });
  }
  return {specs, skipped, maxHeight};
}
...
export function reconcile(specs: ContainerSpec[], previous: Layout, cols: number, rows: number, maxHeight: number): Layout {
  ...
  for (const raw of specs) {
    ...
    const spec=unchanged?{...raw,height_u:Math.max(minHeight(raw),Math.min(maxHeight,before.height_u))}:raw;
